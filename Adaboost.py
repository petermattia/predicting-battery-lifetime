# -*- coding: utf-8 -*-
"""
Created on Mon Dec 10 02:43:20 2018
...
#    use_log_cycle_life = False    
    ...
    best_learning_rate = np.zeros(N_cycles.shape)    
    trained_models = []
...
#        learning_rate = np.array([0.001,0.003,0.01,0.1])
        ...
        mpe = np.zeros([len(n_trees),len(learning_rate)])        


        
        for j in np.arange(len(n_trees)):
            for k in np.arange(len(learning_rate)):
                # AB
        #        my_AB = sk.svm.AB(kernel='rbf', degree=3, gamma='auto_deprecated', coef0=0.0, tol=0.001, C=1.0, learning_rateilon=0.1, shrinking=True, cache_size=200, verbose=False, max_iter=1000)
                my_AB = AdaBoostRegressor(n_estimators=n_trees[j], learning_rate=learning_rate[k])
                print('n_trees = ' + str(n_trees[j]))
                print('learning_rate = ' + str(learning_rate[k]))
                

                my_AB.fit(features,cycle_lives)
                predicted_cycle_lives = my_AB.predict(features)
                residuals = predicted_cycle_lives - cycle_lives
                train_rmse[j,k] = np.sqrt(((residuals) ** 2).mean())
                mse = -cross_val_score(my_AB, features, cycle_lives, cv=5, scoring='mean_squared_error')
                rmse[j,k] = np.sqrt(abs(np.mean(mse)))
                next_mpe = cross_val_score(my_AB, features, np.log10(cycle_lives), cv=5, scoring=mean_percent_error)
...
#        best_learning_rate[i] = learning_rate[best_index[1]]
        index_best = np.argmin(rmse)
        best_n_trees_index = index_best // len(learning_rate)
        print('Best n_trees index ' + str(int(best_n_trees_index)))
        best_n_trees[i] = n_trees[best_n_trees_index]
        
        best_learning_rate_index = np.mod(index_best, len(learning_rate))
        print('Best learning_rate index ' + str(int(best_learning_rate_index)))
        best_learning_rate[i] = learning_rate[best_learning_rate_index]

        print('Best learning_rate:')
        print(best_learning_rate[i])
        print('Best n_trees:')
        ...
        best_AB = AdaBoostRegressor(n_estimators=n_trees[j], learning_rate=learning_rate[k])
        best_AB.fit(features,cycle_lives)
        trained_models.append(best_AB)
        
        predicted_cycle_lives = best_AB.predict(features)
        residuals = predicted_cycle_lives - cycle_lives
...
#    df.to_csv("norm_coeffs.csv")
    ...
    pickle.dump(trained_models, open('AB_trained_models.pkl', 'wb'))
    pickle.dump(min_mpe, open('AB_crossvalid_percenterror.pkl', 'wb'))  
    pickle.dump(training_mpe, open('AB_training_percenterror.pkl', 'wb'))    
